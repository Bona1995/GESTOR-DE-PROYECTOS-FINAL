...
from negocio.negocio import (iniciarDatos, agregarProducto, verProducto, obtenerNombre, obtenerCategoria, obtenerRango, eliminacionRegistro, modificacionPrecio, modificacionCantidad, obtenerCantidad,)
...
def menu():
    ...
    while True: # Bucle iterativo hasta BREAK, permite seleccionar opciones con MATCH, vuelve si se selecciona menos de 1 o mas de 5.
        ...
        match opcionMenu: #Selector de menu de opciones
            case 1: 
                while True:
                    print(Back.YELLOW + "AGREGAR PRODUCTO.\n")
            ...
            case 2:
                while True:
                    print(Back.YELLOW + "VER PRODUCTO.\n")
            ...
            case 3:
                while True:   
                    print(Back.YELLOW + "BUSCAR PRODUCTO.\n")
                    resultadoBuscar= buscarProductosMenu()
                    print(f"Finalizando busqueda de productos.\n....\n...\n..\n.")
                    break 
            case 4:
                while True:
                    print(Back.YELLOW + "ELIMINAR PRODUCTOS.\n")
                    resultadoEliminar= eliminarProducto()
            ...
            case 5:
                 while True:
                    print(Back.YELLOW + "MODIFICAR PRODUCTOS\n")
                    resultadoModicar= modificarProductosMenu()
                    print(f"Finalizando busqueda de productos.\n....\n...\n..\n.")
                    break 
            case 6:
                print(Back.YELLOW + "SALIR DEL PROGRAMA.\n")
                print("Desea salir del programa o volver al menu principal?. \n")
                _selector=input(f"Ingrese `S` para salir y `N` para volver al menu princiapal: ").strip().lower()
                while _selector.isdigit() or (_selector!= "s" and _selector!= "n"):
                    print(Fore.RED + "Error de tipeo, solo S para continuar o N para salir ")
                    _selector=input(f"Ingrese `S` para salir y `N` para volver al menu princiapal: ").strip().lower()
                if _selector == "s":
...
def inicioDatos():
    """Da inicio a la base de datos, y dos diccionarios iniciales"""
    print(Fore.GREEN + "Inicializando la base de datos, aguarde un instante")
...
def agregarProductos():
    ...
    while True:
        ...
        resultadoAgregar= agregarProducto(productoAgregar)
        break
...
def verProductos():
    ...
    print("A continuacion podra ver todos lo productos diponibles. \n")
    while True: 
        resultadoVer= verProducto()       
        centinelaInfo= ""
        if resultadoVer:
...
def buscarProductosMenu():
    ...
    while True:   
        ...
        selector= validarSelectorBusquedaMenu()
        match selector:
            case 1: 
...
def modificarProductosMenu():
    """Selecciona que modificar de un producto"""
    while True:   
        print("Que desea modificar?.\n")
        print("1: Precio")
        print("2: Stock")
        print("3: Salir a menu principal")
        selector= validarSelectorModificacionMenu()
        match selector:
            case 1: 
                print("MODIFICAR PRECIO")
                while True:
                    centinelaInfo= modificarPrecio()
                    print (f"{centinelaInfo}")
                    ...
                    if selectorSalida == "s":
                        ...
                        print("Finalizando modificacion de productos.\n....\n...\n..\n.")
                        break 
            case 2: 
                print("MODIFICAR STOCK")
                while True:
                    centinelaInfo= modificarCantidad()
                    print (f"{centinelaInfo}")
...
def busquedaNombre():
    while True:
        ...
        if nombreBuscar == "" or nombreBuscar.isdigit():
            print(Fore.RED + "Error de tipeo, vuelva a intentarlo")
            continue
        else:
            break
    ...
    if resultadoNombre:           
        centinelaInfo= (Fore.RED + f"El producto que busco: ID: {resultadoNombre[0]}, NOMBRE: {resultadoNombre[1]}, DESCRIPCION: {resultadoNombre[2]}, CANTIDAD: {resultadoNombre[3]}, PRECIO: {resultadoNombre[4]}, CATEGORIA: {resultadoNombre[5]}.\n")
    else:
        centinelaInfo= (Fore.RED + f"No se encontraron datos que coincidan con el nombre: {nombreBuscar}")
...
def busquedaCategoria():
    ...
    if resultadoCategoria:
        ...
        centinelaInfo= (Fore.RED + f"No se encontro ningun producto en la categoria: {categoriaBuscar}.\n")
...
def busquedaRango():
    while True:
        ...
        if rangoBuscarMin > rangoBuscarMax:
            print(Fore.RED + "Error: El precio minimo no puede ser mayor que el precio maximo. Vuelva a intentarlo.")
            continue
        else:
            break
    ...
    if resultadoRango:
        ...
        centinelaInfo= (Fore.RED + f"No se encontro ningun producto en la categoria con esos parametros: {rangoBuscarMin} y {rangoBuscarMax}.\n")
...
def eliminarProducto():
    while True:
        segunPosicion= input("Ingrese el ID del producto a eliminar: .\n").strip().title()  
        if (segunPosicion == "") or (not segunPosicion.isdigit()):
            continue
        else:
            break
    centinelaInfo= None
    resultadoEliminacion= eliminacionRegistro(segunPosicion)
    if resultadoEliminacion:
        print(Fore.GREEN + "Producto eliminado exitosamente. \n")
        centinelaInfo= resultadoEliminacion
    else:
        centinelaInfo= (Fore.RED + "No se encontro ningun producto a eliminar.\n")
...
def modificarCantidad():
    while True:
        segunPosicion= input("Ingrese el ID del producto a modificar").strip().title()
        modificCantidad= input("Ingrese el stock del producto a modificar").strip().title()  

        if (segunPosicion == "") or (not segunPosicion.isdigit()) or (modificCantidad == "") or (not modificCantidad.isdigit()):
            print(Fore.RED + "Error de tipeo, vuelva a intentarlo")
            continue
        else:
            break
    resultadoModificarCantidad= modificacionCantidad(modificCantidad, segunPosicion)
    if resultadoModificarCantidad:
        print(Fore.GREEN + "Datos modificados exitosamente")
        centinelaInfo= resultadoModificarCantidad
    else:
        centinelaInfo= (Fore.RED + "No se encontro ningun producto a modificar.\n")
...
def modificarPrecio():
    while True:
        segunPosicion= input("Ingrese el ID del producto a modificar").strip().title()
        nuevoPrecio= input("Ingrese el precio del producto a modificar").strip().title()  

        if (segunPosicion == "") or (not segunPosicion.isdigit()) or (nuevoPrecio == "") or (not nuevoPrecio.isdigit()):
            print(Fore.RED + "Error de tipeo, vuelva a intentarlo")
            continue
        else:
            break
    resultadoModificarPrecio= modificacionPrecio(nuevoPrecio, segunPosicion)
    if resultadoModificarPrecio:
        print(Fore.GREEN + "Datos modificados exitosamente")
        centinelaInfo= resultadoModificarPrecio
    else:
        centinelaInfo= (Fore.RED + "No se encontro ningun producto a modificar.\n")
...
def validarSelectorMenu(minimo=1, maximo=6): # Parametros predefinidos, se pueden pasar los argumentos al ser llamada para usar en otro menu o seleccion conn numero entero.
    ...
    while True:
        ...
        if not opciones.isdigit():
            print(Fore.RED + "Ingreso de opcion invalido, vuelva a intentarlo.")
            continue 
        ...
        if (opcion_int < minimo  or opcion_int > maximo): # Validamos solo rangos entre los paramatros 1 a 5
            print(Fore.RED + "Ingreso de opcion fuera de rango, vuelva a intentarlo.")
            continue
        else:
            break
    return opcion_int
...
def validarSelectorModificacionMenu(minimo=1, maximo=3): # Parametros predefinidos, se pueden pasar los argumentos al ser llamada para usar en otro menu o seleccion conn numero entero.
    """Validar la seleccion ingresada por el usuario, deben ser numero enteros entre el maximo y minimo del menu. Luego castea a entero"""
    while True:
        opciones= input(f"Seleccionar una opción del {minimo} al {maximo}: ")
        if not opciones.isdigit():
